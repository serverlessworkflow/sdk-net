    ...
    : TaskExecutor<DoTaskDefinition>(serviceProvider, logger, executionContextFactory, executorFactory, schemaHandlerProvider, task)
{
    ...
    protected override async Task ExecuteCoreAsync(CancellationToken cancellationToken)
    {
        ...
        if (nextEntry == null)
        {
            await SetResultAsync(last?.State.Output, Task.Definition.Then, cancellationToken).ConfigureAwait(false);
            return;
        }
        var nextIndex = Tasks.Keys.ToList().IndexOf(nextEntry.Key);
        var input = last == null ? Task.Input : last.State.Output ?? new JsonObject();
        ...
        var executor = await CreateTaskExecutorAsync(next, nextEntry.Value, Task.ContextData, Task.Arguments, cancellationToken).ConfigureAwait(false);
    ...
    async Task OnSubtaskCompletedAsync(ITaskExecutor executor, CancellationToken cancellationToken)
    {
        ...
        if (nextEntry == null)
        {
            var then = lastState.Status != TaskInstanceStatus.Skipped && lastState.Next == FlowDirective.End ? FlowDirective.End : Task.Definition.Then;
        ...
        switch (flowDirective)
        {
            ...
            default:
                ...
                var nextExecutor = await CreateTaskExecutorAsync(next, nextEntry.Value, Task.ContextData, Task.Arguments, cancellationToken).ConfigureAwait(false);
