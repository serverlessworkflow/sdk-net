...
public sealed class ForTaskExecutor(IServiceProvider serviceProvider, ILogger<ForTaskExecutor> logger, ITaskProcessFactory taskProcessFactory, ITaskExecutorFactory executorFactory, ISchemaHandlerProvider schemaHandlerProvider, ITaskProcess<ForTaskDefinition> task)
    : TaskExecutor<ForTaskDefinition>(serviceProvider, logger, taskProcessFactory, executorFactory, schemaHandlerProvider, task)
{
    ...
    {
        var result = await Task.Workflow.Expressions.EvaluateAsync(Task.Instance.Definition.For.In, Task.Instance.State.Input, GetExpressionEvaluationArguments(), cancellationToken).ConfigureAwait(false);
    ...
    protected override async Task ExecuteCoreAsync(CancellationToken cancellationToken)
    {
        ...
        var taskDefinition = new DoTaskDefinition() { Do = Task.Instance.Definition.Do };
        var taskInstance = await Task.Workflow.Instance.CreateTaskAsync(taskDefinition, GetPathFor(index.ToString()), Task.Instance.State.Input, null, Task.Instance, false, cancellationToken).ConfigureAwait(false);
        var contextData = Task.Instance.State.ContextData.DeepClone().AsObject()!;
        var arguments = Task.Arguments.DeepClone().AsObject()!;
        arguments[Task.Instance.Definition.For.Each] = item?.DeepClone();
        arguments[Task.Instance.Definition.For.At ?? RuntimeExpressions.Arguments.Index] = index;
    ...
    async Task OnIterationCompletedAsync(ITaskExecutor executor, CancellationToken cancellationToken)
    {
        ...
        switch (executor.Task.Instance.State.Next)
        {
            ...
            case FlowDirective.Exit:
                ...
                break;
            default:
                await SetErrorAsync(Error.Configuration(new Uri(Task.Instance.State.Reference.ToString(), UriKind.RelativeOrAbsolute), "Unable to continue with a specific task within a loop"), cancellationToken).ConfigureAwait(false);
                break;
        }
    }
